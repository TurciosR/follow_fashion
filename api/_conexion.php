...
function edad($fecha)
{
    if($fecha !="0000-00-00")
    {
      list($anion, $mesn, $dian) = explode("-", $fecha);
...
function mora($mes, $anio)
{
  $mora = 0;
  $sql = _query("SELECT dia_vence, mora FROM configuracion WHERE id_configuracion='1'");
  $datos = _fetch_array($sql);
  $ultimo = $datos["dia_vence"];
  $fi = $anio."-".$mes."-".$ultimo;
  $fa = date("Y-m-d");
  if(strtotime($fi) < strtotime($fa))
  {
    $mora = $datos["mora"];
  }
  return $mora;
...
function escribir($cadena,$archivo){
...
function _get_file_path($prefix, $url){
    ...
    $result = explode("/", $url);
    return $prefix.$result[sizeof($result)-1];
}
