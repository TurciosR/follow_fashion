...
function print_ticket($id_factura)
{
    ...
    if($nrows_fact>0){
        ...
        $info_factura.="TEL: $field[telefono1]"."\n";
        if (trim($field['telefono2'])!="")
        {
...
//========================== PRINT TICKET CORTE DE PARQUEO =====================
function print_ticket_corte_parqueo($infoCorte)
{
    $id_sucursal=$_SESSION['id_sucursal'];
    //Valido el sistema operativo y lo devuelvo para saber a que puerto redireccionar
    $info = $_SERVER['HTTP_USER_AGENT'];
    ...
    $info_factura="";
    $tinicio= zfill($infoCorte['correlativo_inicio'], 7);
    $tfinal= zfill($infoCorte['correlativo_fin'], 7);
    $empresa=empresa();

        //Columnas y posiciones base
    //$info_factura.=$esp_init0."SUCURSAL ".$nombre_sucursal."\n";
    $info_factura.=$esp_init0."FOLLOW FASHIONS - CORTE DE PARQUEO\n";
    $info_factura.=$esp_init0."CORTE DE CAJA  : ".str_pad(
        $infoCorte['id_corte_parqueo'], 11, "0", STR_PAD_LEFT)."\n";
    $info_factura.=$line1;
    $info_factura.=$esp_init."FECHA: ".$infoCorte['fecha']."  HORA:".hora($infoCorte['hora'])."\n";
    $info_factura.=$esp_init."EMPLEADO: ".$infoCorte['nombre']."\n";
    //-----------------------------
    $info_factura.=$line1;
    $info_factura.=$esp_init0."TIQUETES EMITIDOS:  \n";
    $info_factura.=$esp_init0."INICIO:". str_pad(
        $tinicio, 25, " ", STR_PAD_LEFT)."\n";
    $info_factura.=$esp_init0."FINAL :". str_pad(
        $tfinal, 25, " ", STR_PAD_LEFT)."\n";
    $info_factura.=$esp_init0."TIQUETES ANULADOS:".str_pad(
        $infoCorte['ticket_anulados'], 14, " ", STR_PAD_LEFT)."\n";
    $info_factura.=$line1;
    $n=5;
    $sp1=len_num($infoCorte['total_corte'],$n);

    $info_factura.=$esp_init0."TOTAL CORTE:".str_pad(
        "$".$infoCorte['total_corte'], 20, " ", STR_PAD_LEFT)."\n";
    $sp1=len_num($infoCorte['efectivo_en_caja'],$n);

    $info_factura.=$esp_init0."EFECTIVO EN CAJA:".str_pad(
        "$".number_format($infoCorte['efectivo_en_caja'],2,".",""),15," ",STR_PAD_LEFT
    )."\n";

    $sp1=len_num($infoCorte['diferencia'],$n);
    $info_factura.=$esp_init0."DIFERENCIA:".str_pad(
        "$".$infoCorte['diferencia'], 21, " ", STR_PAD_LEFT)."\n";
...
function print_corte($id_corte)
{
    ...
    if($tipo=="C"){
        ...
        $sql_cuenta = _query("SELECT mc.*, f.numero_doc, f.id_cliente, f.nombre AS nombre_f, c.nombre AS nombre_c
          ...
          WHERE mc.id_apertura = '$row[id_apertura]' AND mc.hora <='$hora' AND mc.numero_doc != ''");
