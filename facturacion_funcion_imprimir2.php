...
function print_fact($id_factura,$tipo_id,$nombreapecte,$direccion){
	...
	if($nrows_fact>0){
		...
		//Datos encabezado factura
		$arrayL[10]=  p_set($arrayL[10],($fecha_fact),16,40,"R");
		if($nodp=="0000-000000-000-0"){$nodp="";}
		$arrayL[10]= p_set($arrayL[10],$nodp,65,99,"R");

		$arrayL[13]= p_set($arrayL[13],$nombre_ape,16,99,"R");

		$arrayL[15]= p_set($arrayL[15],$direccionx,18,52,"R");
		...
		for($i=0;$i<$nrows_fact_det;$i++){
			...
			if ($i<24) {
				...
				if ($exento==0){
					...
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],quitar_spc($descrip),14,60,"R");
	...
	foreach ($array_nocon as $key => $value) {
		// code...
		$arrayL[$array_painc[$key]]=p_set($arrayL[$array_painc[$key]],$value,12,55,"R");
	}

	$arrayL[45]= p_set($arrayL[45],sprintf("%.4f",$SumGravado),85,99,"L");
	$arrayL[48]= p_set($arrayL[48],sprintf("%.4f",$row_fact['retencion']),85,99,"L");
	$arrayL[50]= p_set($arrayL[50],sprintf("%.4f",round(($SumGravado-$row_fact['retencion']),4)),85,99,"L");
	$arrayL[54]= p_set($arrayL[54],sprintf("%.4f",$venta_exenta),85,99,"L");
	$arrayL[56]= p_set($arrayL[56],sprintf("%.2f",round(($SumGravado+$venta_exenta-$row_fact['retencion']),2)),85,99,"L");
...
function print_ccf($id_fact,$tipo_id,$nitcte,$nrccte,$nombreapecte,$direccion){
	...
	if($nrows_fact>0){
		...
		$arrayL[8]=  p_set($arrayL[8],$nombreapecte,11,65,"R");
		$arrayL[8]=  p_set($arrayL[8],$fecha_fact,75,99,"B");

		$arrayL[10]=  p_set($arrayL[10],$dir_txt,15,65,"R");
		$arrayL[10]=  p_set($arrayL[10],$nrccte,80,99,"L");
		$arrayL[12]=  p_set($arrayL[12],$depart,46,67,"R");

		$arrayL[12]=  p_set($arrayL[12],substr($giro_cte1,0,20),79,99,"R");
		$arrayL[13]=  p_set($arrayL[13],substr($giro_cte1,20),79,99,"R");


		$arrayL[14]=  p_set($arrayL[14],$nitcte,44,67,"R");
		$arrayL[14]=  p_set($arrayL[14],get_tipo($row_fact['credito']),85,99,"R");
		...
		for($i=0;$i<$nrows_fact_det;$i++){
			...
			if ($i<23) {
				...
				if ($exento==0){
					...
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],quitar_spc($descrip),12,64,"R");
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],$precio_sin_iva,65,74,"L");
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],number_format(($subtotal/1.13),4,".",""),87,99,"L");
				}
				if ($exento==1){
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],$cantidad,4,10,"B");
					$arrayL[$array_painc[$i]]=p_set($arrayL[$array_painc[$i]],quitar_spc($descrip),12,65,"R");
	...
	$array_nocon= dtl($cadena_salida_txt,48);
	foreach ($array_nocon as $key => $value) {
		// code...

		$arrayL[$array_painc[$key]]=p_set($arrayL[$array_painc[$key]],$value,12,60,"B");
	}
	$arrayL[44] = p_set($arrayL[44],$total_value_gravado,87,99,"L");
	$arrayL[46] = p_set($arrayL[46],"0.0000",87,99,"L");
	$arrayL[48] = p_set($arrayL[48],$total_iva_format,87,99,"L");
	$arrayL[50] = p_set($arrayL[50],$subtotal_gravado_print,87,99,"L");
	$arrayL[52] = p_set($arrayL[52],$retencion,87,99,"L");
	$arrayL[54] = p_set($arrayL[54],$total_value_exento,87,99,"L");
	$arrayL[56] = p_set($arrayL[56],sprintf("%.2f",$total_final_format),87,99,"L");
