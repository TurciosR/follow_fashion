...
if($id_proveedor!=-1)
{
  $sql = _query("SELECT * FROM proveedor where id_proveedor=$id_proveedor");
  while ($row = _fetch_array($sql)) {
    $pname =$row['nombre'];
    $and = " producto.id_proveedor=$id_proveedor AND";
  }
}
...
$pdf->Cell(215, 6, utf8_decode("PROVEEDOR: ".$pname), 0, 1, 'C');
...
$sql_stock = _query("SELECT producto.id_producto, producto.barcode,producto.descripcion,producto.minimo,stock.stock FROM stock JOIN producto WHERE $and stock.id_producto=producto.id_producto AND stock.stock<producto.minimo");
